(function() {
  ...
  var $resourceMinErr = angular.$$minErr('$sailsResource');
  ...
  function shallowClearAndCopy(src, dst) {
    ...
    angular.forEach(dst, function(value, key){
      if (key !== '$ee') { // Prevent deleting EventEmitter
        delete dst[key];
      }
    });
  ...
  function needsPopulate(src, dst) {
    for (var key in src) {
      if (src.hasOwnProperty(key) && angular.isObject(dst[key]) && !angular.isObject(src[key])) {
        return true;
      }
    }
    return false;
   ...
   * ```
   */
  angular.module('sailsResource', ['sails.io']).
  ...
  //        'save': {method: 'POST'},
      ...
      this.$get = ['$http', '$q', '$rootScope', '$sailsSocket', function ($http, $q, $rootScope, $sailsSocket) {
        ...
        function resourceFactory(model, url, paramDefaults, actions, options) {
          ...
          forEach(actions, function (action, name) {
            ...
            Resource[name] = function (a1, a2, a3, a4) {
              ...
              var urlParams = extend({}, extractParams(data, action.params || {}), params);

              var isInstanceCall = this instanceof Resource;
              var value;
              var cacheKey = action.isArray ? JSON.stringify(urlParams || {}) : urlParams.id; // cache key is params for lists, id for items
              if (isInstanceCall) {
                 value = data;
              } else {
                if (action.cache) {
                  value = action.isArray ? (cache[cacheKey] || []) : (cache[cacheKey] || new Resource(data));
                } else {
                  value = action.isArray ? [] : new Resource(data);
                }
                // Remove old $promise if exists
                delete value.$promise;
                delete value.$resolved;
              }

              if (action.cache === undefined || action.cache === true) {
            ...
            Resource.prototype.$off = function() {
              this.$ee.removeListener.apply(this.$ee, arguments);
            };

            Resource.prototype.$trigger = function() {
            ...
            forEach(cache, function (cacheItem, key) {
              if (angular.isArray(cacheItem)) {
                forEach(cacheItem, function (item) {
                  if (item.id == message.id) {
                    if (needsPopulate(message.data, item)) { // go to server for updated data
                      var promise = item.$findOne({ id: item.id });
                      promise.then(function(resource) {
                        resource.$trigger('reloaded', resource);
                      });
                      promises.push(promise);
                    }
                    else {
                      shallowClearAndCopy(message.data, item);
                      item.$trigger('updated', item);
                    }
                  }
                });
              }
                ...
                if (needsPopulate(message.data, cacheItem)) { // go to server for updated data
                  var promise = cacheItem.$findOne({ id: cacheItem.id });
                  promise.then(function(resource) {
                    resource.$trigger('reloaded', resource);
                  });
                  promises.push(promise);
                  ...
                  shallowClearAndCopy(message.data, cacheItem);
                  cacheItem.$trigger('updated', cacheItem);
                }
              }
            });
            return promises;
          ...
          function socketReloadResource(id) {
            var promises = [];
            forEach(cache, function (cacheItem, key) {
              if (angular.isArray(cacheItem)) {
                forEach(cacheItem, function (item) {
                  if (item.id == id) {
                    var promise = item.$findOne({ id: id });
                    promise.then(function(resource) {
                      resource.$trigger('reloaded', resource);
                    });
                    promises.push(promise);
                ...
                var promise = cacheItem.$findOne({ id: id });
                promise.then(function(resource) {
                  resource.$trigger('reloaded', resource);
                });
                promises.push(promise);
          ...
          function socketAddedToResourceAttribute(message) {
            // TODO: Can be more smart instead of just reload item?
            return socketReloadResource(message.id);
          }

          function socketRemovedFromResourceAttribute(message) {
            // TODO: Can be more smart instead of just reload item?
            return socketReloadResource(message.id);
          }

          // Subscribe to changes
          $sailsSocket.subscribe(model, function (message) {
            ...
            $rootScope.$apply(function () {
              ...
              switch (message.verb) {
                case 'updated':
                  promises = socketUpdateResource(message);
                  messageName = MESSAGES.updated;
                  break;
                case 'created':
                  promises = socketCreateResource(message);
                ...
                case 'messaged':
                  ...
                  break;
                case 'addedTo':
                  promises = socketAddedToResourceAttribute(message);
                  messageName = MESSAGES.addedTo;
                  break;
                case 'removedFrom':
                  promises = socketRemovedFromResourceAttribute(message);
                  messageName = MESSAGES.removedFrom;
                  break;
              }
              message.model = model;
              if (promises && promises.length) {
                message.$promise = $q.all(promises);
              }
